...
import { generateInvoice } from './orderInvoice/generateInvoice'

const createOrderFromCart = async (userID: number, orderData:any) => {
   const connection = await db.getConnection();
   try{

    await connection.beginTransaction();
    const [addressId]:any = await connection.query(`select address.address_id from address inner join user_address on address.address_id = user_address.address_id 
               where user_id = ? and user_address_status = ?`,[userID, orderData.address_status])
    if (addressId.length === 0) {
        throw new Error("Address not found");
        }

     const [cart]:any = await connection.query('select * from cart where user_id = ?',[userID])
     if (cart.length === 0) {
        throw new Error("Cart not found");
        }
    const cart_id = cart[0].cart_id
     
     const [cartItem]:any = await connection.query('select * from cart_items where cart_id = ?',[cart_id])
    ...
    const ordersTableplaceholder = ordersTableValue.map(()=>'?').join(",")
     ...
     const [orderTableResult] = await connection.query<ResultSetHeader>(orderQuery,[...ordersTableValue])
      ...
      for (const item of cartItem) {
        const [variantStock]: any = await connection.query('SELECT stock FROM product_variants WHERE variant_id = ?',[item.variant_id])

        if(item.quantity > variantStock[0].stock){
         throw new Error("Out of stock");
        }

     const [price]:any = await connection.query('select price from product_variants where product_id = ? and variant_id = ?',[item.product_id, item.variant_id])
     console.log(  [price] )
       ...
       const orderItemQuery = `insert into order_items ( ${ordersItemTableKey} ) values ( ${ordersItemTableplaceholder} )`;
        await connection.query<ResultSetHeader>(orderItemQuery,[...ordersItemTableValue])
        
      const [discountPercentage]:any = await connection.query('select discount_percentage from product_discounts where product_id = ?', [productID])
      ...
      const amountWithDiscount = (( item.quantity as number) * (price[0].price as number)) * ( 1 - (discount / 100 ));
       totalPurchaseAmount = totalPurchaseAmount + amountWithDiscount
        ...
        const finalAmount = totalPurchaseAmount + taxAmount;
         ...
         const [paymentMethodID]:any = await connection.query('select method_id from payment_methods where method_name = ?',[orderData.payment_method_name])
        ...
        await connection.query<ResultSetHeader>(paymentTableQuery,[...paymentTableValue])

      await connection.query('DELETE FROM cart_items WHERE cart_id = ?',[cart_id]);

      const [users]:any = await connection.query('select * from users where user_id = ?',[userID])
      ...
      console.log("Generating invoice...");
       const invoicePath = generateInvoice(orderDataPDf);
